...
URL만 보고도 홈 화면임을 알 수 있도록 /home 경로를 명시적으로 두었습니다.

추후 /에서 인증 체크나 진입 분기 같은 별도 로직이 필요해질 경우를 대비해, /와 /home의 책임을 미리 분리해두었습니다.
...
매직넘버 eslint error -> warn으로 변경

- error로 commit시 오류로 걸리는 현상 -> 이를 해소하기 위해 매번 예외 넘버를 넣어줘야 하는 번거로움이 발생하여 warn으로 변경하였습니다.
