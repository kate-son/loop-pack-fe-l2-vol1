...
**판정**: 삭제 대상이 `entities/wishlist`, `features/toggle-wishlist` 두 슬라이스로 응집돼 있고, 수정 대상도 "위시리스트를 소비하는 쪽"(Header, Body) 4곳으로 grep(`wishlist`, `Wishlist`) 한 번이면 다 찾을 수 있다. `entities/product`(`ProductCard` 포함)·`features/product-filter`·페이지 쪽은 안 건드려도 될 것으로 보인다.
