...
- **결정**: 스크롤 동작을 바꾸는 대신 **히어로 높이를 줄여 이동 폭 자체를 줄이는 방향**을 택했다.
...
- **결정**: 검색 중 화면이 튀는 체감이 나쁘다는 판단에 따라 **검색에서는 스크롤을 유지하기로 했다.**
...
1부 "결정" 절에서 "스켈레톤을 쓰지 않는다"고 정했었는데, **그 결정을 번복했다.** "전체 화면이 아니라 목록 영역만 채우는 것이므로 FCP 부담이 크지 않다"는 판단과 함께 **스켈레톤 도입을 결정**했고, 아래는 그 판단을 뒷받침하는 재검토다.
...
> **결론부터**: 이 라운드의 코드 변경은 **되돌렸다.** `next.config.ts`는 Part 1 종료 시점 그대로다. **철회는 리뷰 과정에서 "캐시 때문이 아니라고 본다"는 판단에 따라 결정했다.** 아래는 무엇을 근거로 시도했고 왜 접었는지의 기록이다 — 그 과정에서 얻은 실측 하나는 계속 쓸모가 있어 남긴다.
...
`images.minimumCacheTTL`을 기본 60초 → 1일로 늘렸다가 되돌렸다. **"느려진 게 캐시 때문은 아니다"고 판단**했고, 근거는 넷이다.
...
진단에서 확인한 "hero와 카드 이미지의 대역폭 경쟁"을 실제로 고쳤다. 원래는 Part 1 주제라 관찰 기록으로만 남기려 했으나, **"배너를 먼저 보여주는 쪽"을 택하기로 결정**해 이 Part에서 처리했다.
