...
`/products`에서 목록이 바뀌는 동안 사용자가 지금 무슨 상태인지 알 수 있게 만드는 작업. Before 관찰에서 6개 상태 중 3개가 미충족이었고, Round 0–10을 거쳐 **6개 모두 충족**시켰다.
...
> | Round 8   | Speed Index 개선 시도(미측정)                        | — (유일하게 성능이 목적인 라운드)                          |
...
| 현재 코드        | Round 0–10 적용 후 (**아직 커밋 전** — 변경 파일은 각 Round 절에 명시)                                                                                |
...
| Round 8   | Speed Index 개선 — 이미지 변환 캐시 TTL 60초 → 1일                     | O         | **⚠️ 미측정**      |
| Round 9   | 스켈레톤에 Pagination 자리 추가 (CLS 회귀 수정)                        | O         | **⚠️ 미측정**      |
| Round 10  | 배너 우선 — 카드 이미지 `fetchPriority="low"`(홈·상품목록 공통)        | O         | 참고 측정만        |

### 문서 구성

**1부 Before**(기준선·측정 대상·상태별 관찰·설계 결정) → **2부 개선 라운드**(Round 0–10) → **3부 정리**(처리 현황·완료조건 점검·마무리)
...
| 최초 실패               | 목록 대신 실패 이유·재시도 방법                | 코드상 충족되나 실사용 경로로 **재현 불가**                                | ✅ 코드 구조 유지 + 회귀 테스트로 고정(**Round 2**) — 실측 재현은 여전히 못 함 |
...
### 1) 데이터 없는 최초 진입 — 구조적으로 거의 재현 불가능
...
- `/products`의 `page.tsx`가 `await queryClient.prefetchQuery(...)`로 **document 응답 자체**를 데이터가 준비될 때까지 막고, `HydrationBoundary`로 넘겨준다 — 그래서 하드 리로드(cold load)로는 클라이언트가 마운트되는 시점에 이미 `data`가 존재해 `isPending`이 절대 `true`가 되지 않는다.
...
**결론**: 정상적인 사용자 여정(하드 리로드, 필터 변경) 어느 경로로도 `isPending===true`가 실제로 노출되지 않는다. 이 gap은 "지금 당장 사용자가 겪는 문제"가 아니라, **향후 SSR prefetch 없는 라우트(예: 클라이언트 전용 모달, CSR-only 진입점)가 추가될 때를 위한 안전망 부재**로 봐야 한다.
...
1부 "결정" 절에서 "스켈레톤을 쓰지 않는다"고 정했었는데, **그 결정을 번복했다.** 번복 근거를 먼저 남긴다.
...
**앞으로의 측정 규칙에 추가**: 라운드 간 LCP를 비교하려면 ① **URL을 고정**하고 ② **이미지 캐시 상태를 명시**해야 한다(예열 후 / 비운 직후 중 하나로 통일). Round 8의 `minimumCacheTTL` 연장은 이 교란 요인 자체를 크게 줄여준다.
...
## Round 8 — Speed Index 개선: 이미지 변환 결과 캐시 TTL 연장

### 왜 Speed Index였나

Round 7 측정에서 **Speed Index 중앙값 2,284ms**로, 같은 리포트의 FCP·LCP보다 점수가 낮았다(Round 0 리포트 기준 score 0.43). Speed Index는 "화면이 시각적으로 얼마나 빨리 채워지는가"를 필름스트립 기반으로 적분한 값이라 **뷰포트에서 큰 면적을 차지하는 요소가 늦게 뜨면 크게 나빠진다.** 이 페이지에서 그 요소는 명백히 hero 이미지다(1200×514px, 첫 화면의 절반 이상).

### 관찰 → 가설 → 근거

- **관찰**: Part 1 "LCP breakdown"에서 hero의 `Resource load duration`이 **홈 2,684ms / 상품목록 2,560ms**로 4개 구간 중 압도적으로 컸다. 나머지 3개 구간(TTFB·요청 시작 대기·렌더)을 다 합쳐도 700ms 남짓이다.
- **가설**: 이 구간은 순수 네트워크 전송 시간이 아니다. hero는 원본 7.5MB에서 **요청 시점에 서버가 리사이즈 + AVIF 재인코딩**해서 만들어지는데(Part 1 Round 0의 "원본 기반" 결정), 그 변환 시간이 여기 포함된다. 실제로 최종 전송량은 167KB뿐인데 2.5초가 걸린다는 게 이 가설의 근거다.
- **반증 방법**: 변환 결과가 캐시된 상태와 아닌 상태의 응답 시간을 직접 비교한다. 캐시 히트일 때도 여전히 느리면 가설은 틀린 것이다.

### 실측 — 변환 비용이 응답 시간의 대부분이었다
...
**약 145배 차이**다. 즉 이 응답 시간의 거의 전부가 네트워크가 아니라 **서버의 리사이즈·재인코딩 비용**이며, 가설이 확인됐다(반증 실패).

### 변경

`next.config.ts`의 `images.minimumCacheTTL`을 **기본 60초 → 1일**로 늘렸다.

Next.js는 최적화된 이미지를 `.next/cache/images`에 저장하는데, 기본 TTL이 60초라 **1분만 지나도 만료돼 같은 이미지를 다시 인코딩**한다. 1일로 늘리면 그동안 만든 변환 결과를 계속 재사용한다. 응답 헤더도 `Cache-Control: public, max-age=86400, must-revalidate`로 바뀌어 브라우저·CDN 캐시에도 함께 반영된다.

**왜 1일인가 — 캐시 히트율과 이미지 교체 반영의 트레이드오프**

처음엔 30일로 잡았다가 1일로 낮췄다. `next/image`의 캐시 키는 **소스 URL + width + quality**라서, 어드민이 같은 경로(`/images/week-07/hero-original.jpg`)에 다른 이미지를 덮어써도 **키가 그대로라 TTL이 만료될 때까지 옛 이미지가 계속 나간다.** 게다가 이 값은 브라우저·CDN에 내려가는 `Cache-Control: max-age`에도 그대로 반영되므로, 서버 캐시를 비워도 이미 배포된 응답은 회수할 수 없다.

TTL을 길게 잡을수록 히트율(=성능)은 오르지만 **교체 반영 지연도 같이 길어진다.** 이미지가 바뀔 수 있는 서비스에서 30일은 과했다고 판단해, 최악의 경우에도 하루 안에는 반영되도록 1일로 정했다. 더 긴 TTL이 필요해지면 파일명에 콘텐츠 해시나 버전 쿼리를 붙여 **교체 시 캐시 키 자체가 바뀌게** 만드는 게 정공법이지만, 이번 범위에서는 다루지 않았다.

### 한계 — 첫 요청은 여전히 느리다

이 변경은 **캐시 히트를 늘릴 뿐, 최초 변환 비용(232ms)을 없애지는 못한다.** 따라서:

- 서버 캐시가 비어 있는 상태의 첫 방문자에게는 효과가 없다.
- 반대로 **실사용 환경(여러 사용자가 같은 hero를 반복 요청)에서는 거의 항상 히트**하므로 효과가 크다.
- Lighthouse로 재측정할 때도 **캐시 상태에 따라 결과가 갈린다** — 캐시를 비우고 재는 것과 예열 후 재는 것을 구분해야 한다(아래 측정 방법).

### 측정 방법 (아직 재측정 안 함)

Speed Index가 실제로 줄었는지 확인하려면 **캐시 상태를 명시적으로 나눠서** 재야 한다.

1. **예열 후**(실사용에 가까운 조건): `pnpm build && pnpm start` → 브라우저로 `/products`를 한 번 열어 hero 변환을 캐시에 채운다 → 그다음 Lighthouse 5회. Round 7의 Speed Index 중앙값 **2,284ms**와 비교한다.
2. **캐시 비운 직후**(최악 조건): `rm -rf .next/cache/images` 후 곧바로 Lighthouse — 이 값은 개선되지 않는 게 정상이며, "첫 요청은 여전히 느리다"는 위 한계를 확인하는 용도다.

두 조건을 같은 문서에 나란히 남기면 이 변경의 효과 범위가 정확히 드러난다.
...
바로 위 진단에서 확인한 "hero와 카드 이미지의 대역폭 경쟁"을 실제로 고쳤다. **hero를 먼저 보여주는 쪽을 택한 결정**이다.
...
| 라운드                    | 상태                                                       | 판단                                                                         |
| ------------------------- | ---------------------------------------------------------- | ---------------------------------------------------------------------------- |
| Round 8 (이미지 캐시 TTL) | 예열 후/캐시 비운 직후 Speed Index 비교 미실시             | 성능 목적 라운드로, Part 2 완료조건과 무관해 보류                            |
