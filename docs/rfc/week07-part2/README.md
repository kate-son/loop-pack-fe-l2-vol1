...
`/products`에서 목록이 바뀌는 동안 사용자가 지금 무슨 상태인지 알 수 있게 만드는 작업. Before 관찰에서 6개 상태 중 3개가 미충족이었고, Round 0–11을 거쳐 **6개 모두 충족**시켰다.
...
> | Round 11  | hero 전송량 −34.8%(측정 필요)                        | — (성능 목적)                                              |
>
...
| 현재 코드        | Round 0–11 적용 후 (**아직 커밋 전** — 변경 파일은 각 Round 절에 명시)                                                                                |
...
| Round 11  | hero quality `75` → `65` (전송량 −34.8%)                               | O         | **⚠️ 사용자 측정 필요** |

### 문서 구성

**1부 Before**(기준선·측정 대상·상태별 관찰·설계 결정) → **2부 개선 라운드**(Round 0–11) → **3부 정리**(처리 현황·완료조건 점검·마무리)
...
경쟁을 없앨 수 없다면 **hero 자체를 가볍게 만드는 것**이 남은 방법이라고 직접 판단했다. `w=2400` 변형의 quality별 실측:
...
- `PageHeading`의 hero `<Image>`에 `quality={65}` 적용(`HERO_IMAGE_QUALITY` 상수).
- `next.config.ts`에 `images.qualities: [65, 75]` 추가 — Next 16은 여기 나열된 값만 허용하며, 카드 이미지가 쓰는 기본값 75도 함께 남겼다.
- 홈·상품목록 모두 같은 `PageHeading`을 쓰므로 함께 적용된다(두 페이지 HTML에서 `q=65` 확인).
...
**확실한 것은 하나**: `w=2400`에서 전송량이 **167,195 → 108,965 bytes(−34.8%)**로 줄었다는 직접 측정값이며, 이는 실제 환경에 그대로 적용된다. LCP·Speed Index에 실제로 얼마나 반영되는지는 사용자 측정으로 확인한다(75점 대비).
