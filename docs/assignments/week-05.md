...
- [x] 기본 과제를 먼저 완성했는가
- [x] 선택한 항목과 선택 이유를 기록했는가
- [x] 추가한 복잡도와 검증 결과를 설명할 수 있는가
- [x] A 선택 시 persist·hydration·version·migration을 검증했는가
- [x] B 선택 시 요청별 QueryClient·prefetch·hydration·중복 요청을 검증했는가
- [x] C 선택 시 요청 흐름과 사용자 경험의 개선을 검증했는가
- [x] D 선택 시 핵심 상태 계약을 자동화 테스트로 보호했는가

**공통**

- [x] 변경마다 “왜 이렇게 설계했는가” 한 줄 근거가 있는가
- [x] 개발 중 `pnpm test`와 제출 전 `pnpm check`가 통과하는가
- [x] AI로 생성한 부분을 표기하고 직접 검토했는가
